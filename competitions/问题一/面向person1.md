...
| 指标 | 数值 | 解读 |
|------|------|------|
| 总体一致率 | 10.2% | 模型约1/10周次与淘汰一致 |
| 一致周次数 | 28 / 274 | 符合约束的淘汰事件数 |
| 确定性指标 | 0.995 | 估算高度稳定 |

**分析**：
- 一致率较低表明粉丝投票存在大量难以预测的随机性
- 这符合预期：如果投票可以被准确预测，节目就不会有悬念
- 争议选手（如Bobby Bones）的存在印证了评委与观众意见分歧
- 模型提供的是"可能的投票分布"而非精确预测
