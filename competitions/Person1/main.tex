...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{Q4_fig5_dwvs_group_impact.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.56\textwidth]{Q4_fig2_system_comparison.pdf}
...
\textbf{For Problem 3}, using \textbf{ANOVA} and \textbf{Random Forest}, we find external factors like age or industry have limited predictive power ($R^2=0.11$). Controversies stem not from demographic bias but a \textbf{static weight balance} that lets fanbases override judges in finals.

% --- EXECUTIVE SUMMARY ---
% Section titles are created with \noindent\textbf{...} to avoid indentation and standard section spacing.
\noindent\textbf{Executive Summary}

\noindent % Ensure the paragraph starts without indentation.
Dancing with the Stars thrives on a unique tension: the balance between professional excellence and audience participation. However, recent controversial outcomes—most notably the Season 27 victory of Bobby Bones despite consistently low technical scores—have exposed a structural vulnerability in the current format. Our analysis confirms that the current static 50/50 weighting system allows fan mobilization to completely override professional judgment.

\noindent
To preserve the show’s entertainment value while ensuring the Mirrorball Trophy rewards merit, we recommend transitioning to a Dynamic Weighted Voting System (DWVS). This new framework shifts influence from fans to judges as the season progresses. Simulations confirm this system would have corrected historical anomalies (placing Bobby Bones 2nd) while maintaining audience engagement.

\vspace{1em}

% --- THE CORE PROBLEM ---
\noindent\textbf{The Core Problem: Static Weighting in a Dynamic Competition}

\noindent
Our mathematical reconstruction of 34 seasons reveals that "upset" victories are not random accidents but predictable consequences of the current mechanics:
% Use 'itemize' for unnumbered lists.
\begin{itemize}
    \item \textbf{The "Percentage" Pitfall:} The percentage-based combination method (used in S3-27) disproportionately favors high-popularity/low-skill contestants. Unlike rank-based systems, it allows massive margins in fan votes to wipe out significant deficits in judge scores.
    \item \textbf{The Mobilization Factor:} Analysis shows that external factors like age or industry are less predictive of winning than the efficiency of a star’s fan base. Under the current 50/50 split, a mobilized fan base creates an insurmountable advantage that renders the judges’ technical critiques mathematically irrelevant in the finals.
\end{itemize}

\vspace{1em}

% --- STRATEGIC RECOMMENDATIONS ---
\noindent\textbf{Strategic Recommendations for Future Seasons}

\noindent
We propose the following optimizations for the 2026 season and beyond:
\begin{itemize}
    % Main recommendation point.
    \item \textbf{Implement the Dynamic Weighted Voting System (DWVS)}
        
        \noindent % Text directly under an item, without a sub-list.
        Instead of a static 50/50 split, the weight should shift as the competition intensifies:
        % Use a nested 'itemize' for sub-points to create visual indentation.
        \begin{itemize}
            \item \textit{Early Rounds (Weeks 1-4):} Set fan weight at \textbf{66\%}. This maximizes audience engagement and "fan-favorite" narratives during the season's recruitment phase. % The '%' character is escaped.
            \item \textit{The Progression:} Increase the judge's weighting by \textbf{4\% per week}.
            \item \textit{Finals:} Elevate judge weight to \textbf{74\%}. This ensures that the eventual champion possesses the technical mastery required of a winner, preventing "low-skill, high-popularity" contestants from taking the title.
        \end{itemize}
    
    % Second recommendation point.
    \item \textbf{Introduce a "Threshold Penalty" Mechanism}
    
        \noindent
        To protect the show’s technical standards, we recommend a threshold where if a contestant's judge score falls in the bottom 10\% for three consecutive weeks, their fan vote weight is marginally reduced. This prevents "ironic voting" or meme-driven campaigns from disrupting the competition's integrity.
\end{itemize}

\vspace{1em}

% --- EXPECTED OUTCOMES & ROI ---
\noindent\textbf{Expected Outcomes \& ROI}

\noindent
Our simulations of the proposed DWVS model across historical data demonstrate significant improvements:
\begin{itemize}
    \item \textbf{Controversy Reduction:} The system would have appropriately re-ranked \textbf{78.1\% of historically controversial contestants}. For example, in Season 27, Bobby Bones would have placed \textbf{2nd} instead of winning—acknowledging his popularity while honoring the technical superior.
    \item \textbf{Predictive Stability:} With a cross-season validation score of \textbf{****}, our model ensures that these recommendations are robust against future changes in celebrity casting.
\end{itemize}
...
\noindent
By adopting a dynamic approach to voting weights, \textit{DWTS} can celebrate the fans' voice while upholding the professional standards that make the competition elite. We are prepared to assist your technical team in integrating the DWVS algorithm into your existing voting infrastructure.
