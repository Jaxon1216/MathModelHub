...
- 插值法、chatgpt
- ![alt text](image.png)
- 数据清洗：获取时出现缺失和异常值、噪声、插入（![alt text](image-1.png) 、回归（过一些点）、拉格朗日牛顿插值用多元函数（拉格朗日要过所有点）latex拉格朗日插值、样条插值（相邻两点之间有函数关系![alt text](image-2.png)）、 ![alt text](image-3.png) 、或Matlab(一维二维？)  异常值：![alt text](image-4.png) 判断出来然后剔除补全：正态分布、箱型图（![alt text](image-5.png)）不在极限内认为异常、
- 数据集成（文本型数值型）：
- 数据变换：![alt text](image-8.png)（极大值：成绩、评价；极小值：挂科率；![alt text](image-6.png)
- 数据规约（代表性）：
- 
## 7.参考文献递交格式
