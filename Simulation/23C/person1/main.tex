...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig3_q1_trend.pdf}
\caption{Daily reported results time series with moving averages. The series exhibits classic ``viral decay'' dynamics: rapid growth in early 2022 peaks near 360,000 reports in February, followed by sustained exponential decline to approximately 20,000 by year-end.}
\label{fig:q1_trend}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig4_q1_stl.pdf}
\caption{STL decomposition of reported results. The trend component captures the exponential decay, the seasonal component reveals consistent weekly periodicity, and the residual component represents irregular fluctuations not explained by trend or seasonality.}
\label{fig:q1_stl}
\end{figure}
...
To determine optimal parameters, we first apply a logarithmic transformation to stabilize variance across the declining magnitude of the series. ADF testing on the log-transformed series yields $p = 0.12$, indicating non-stationarity; after first differencing, the test confirms stationarity ($p < 0.001$). The ACF and PACF plots for the differenced series (Fig.~\ref{fig:q1_acf}) exhibit significant autocorrelations at lags 1 and 7, guiding our parameter selection.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig5_q1_acf.pdf}
\caption{Autocorrelation (ACF) and partial autocorrelation (PACF) functions for the differenced log-transformed series. Significant spikes at lag 1 suggest AR(1) and MA(1) components; spikes at lag 7 indicate seasonal AR(1) and MA(1) terms.}
\label{fig:q1_acf}
\end{figure}
...
We assess model adequacy through in-sample fit evaluation and residual diagnostics. Fig.~\ref{fig:q1_fit} compares fitted values against observed data, demonstrating close tracking throughout the series.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig4_q1_fit.pdf}
\caption{SARIMA model fit versus actual reported results. The model successfully captures both the overall declining trend and short-term fluctuations, with fitted values closely following the observed trajectory.}
\label{fig:q1_fit}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig6_q1_forecast.pdf}
\caption{SARIMA forecast through March 2023 with 95\% Bootstrap prediction interval. The shaded region widens progressively, reflecting increasing uncertainty at longer forecast horizons.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig7_q2_heatmap.pdf}
\caption{Correlation heatmap among word features and hard mode ratio. The rightmost column shows correlations with $r_h$; all values are close to zero, indicating negligible linear relationships.}
\label{fig:q2_heatmap}
\end{figure}

The correlation analysis reveals uniformly weak associations between word attributes and hard mode ratio. As shown in Fig.~\ref{fig:q2_barplot}, all Pearson correlation coefficients fall within the range $[-0.05, 0.05]$, and none achieve statistical significance at the $\alpha = 0.05$ level. The Spearman analysis yields substantively identical conclusions, ruling out nonlinear monotonic relationships.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{fig8_q2_barplot.pdf}
\caption{Pearson correlation coefficients between word attributes and hard mode ratio, sorted by absolute magnitude. Gray bars indicate non-significant correlations ($p \geq 0.05$); no features achieve significance.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig9_q3_distribution.pdf}
\caption{Average result distribution across 359 historical words. The distribution is approximately normal with mode at 4 tries; try\_4 accounts for 34\% of outcomes on average, followed by try\_5 (25\%) and try\_3 (23\%).}
\label{fig:q3_dist}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig10_q3_importance.pdf}
\caption{Feature importance for result distribution prediction. Letter frequency metrics dominate, with average letter frequency (0.23) and minimum letter frequency (0.20) as the top predictors. Vowel-related features contribute moderately.}
...
\caption{Predicted result distribution for EERIE (blue bars) compared to historical average (gray bars). Error bars indicate 95\% Bootstrap confidence intervals. EERIE shows elevated try\_2 and try\_3 percentages relative to the historical average.}
\label{fig:q3_eerie}
\end{figure}

\begin{table}[H]
\centering
\caption{EERIE Result Distribution Prediction with 95\% Bootstrap Confidence Intervals ($n=500$)}
...
\caption{Confusion matrix for difficulty classification. The model performs best on Medium and Hard classes, while Easy words are frequently misclassified as Medium due to class imbalance and feature overlap.}
...
\caption{Feature importance for difficulty classification. The top three predictors---average letter frequency, minimum letter frequency, and first letter frequency---collectively account for over 57\% of model importance.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{fig14_q4_eerie.pdf}
\caption{EERIE difficulty classification probabilities. The model assigns 53.2\% probability to Medium, 32.1\% to Hard, and 14.7\% to Easy. The red border highlights the predicted class.}
\label{fig:q4_eerie}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig15_sensitivity.pdf}
\caption{Comprehensive sensitivity comparison across Problem 3 (result distribution) and Problem 4 (difficulty classification) models. The radar plot displays sensitivity indices for key features, where larger values indicate greater model responsiveness to feature perturbations.}
...
\begin{enumerate}
\item \textbf{Difficulty sequencing}: Avoid clustering hard words on consecutive days. Our data suggest that streaks of 3+ hard words correlate with elevated dropout rates. Consider alternating difficulty levels across the week.
\item \textbf{Occasional novelty}: While rare-letter words increase difficulty, they also generate social media discussion. Strategic placement of challenging words (e.g., Fridays) may boost weekend engagement.
\item \textbf{Hard mode monitoring}: Track hard mode completion rates as a quality metric. A completion rate below 80\% may indicate excessive difficulty for this dedicated segment.
\end{enumerate}
