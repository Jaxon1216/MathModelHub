...
\begin{document}
\begin{abstract}
The Olympic medal table reflects national sports capacity, and forecasting the 2028 Los Angeles Games is critical for strategic planning by National Olympic Committees (NOCs). We propose a data-driven framework that integrates regression-based prediction, uncertainty quantification, event-structure analysis, and causal inference for coach mobility. For Problem 1, we build multi-feature regression models and apply Bootstrap resampling to generate prediction intervals for gold and total medals, together with a Poisson model for first-time medalists. For Problem 2, we combine changepoint detection, cross-national medal flow analysis, and a Difference-in-Differences design to identify and quantify the ``Great Coach Effect.'' For Problem 3, we extract policy-relevant insights using concentration metrics and clustering to characterize global medal dynamics. The results show strong predictive performance (R$^2>0.93$), a sizable host advantage, and an average coach effect of roughly 2--5 medals per Olympic cycle in affected sports. These findings support targeted investment strategies and more resilient planning across different NOC types.
...
The Olympic medal table is a key indicator of national sports capacity and resource allocation. In the 2024 Paris Games, the United States led with 126 total medals (40 gold), China tied for most gold (40), host France ranked 5th in gold (16) and 4th in total (64), and Great Britain placed 3rd overall with 65 medals. These outcomes reflect the combined influence of economic capacity, population size, historical tradition, event composition, and home advantage. Meanwhile, first-time medalists (e.g., Albania and Cape Verde) and more than 65 NOCs without any Summer Olympic medals reveal persistent global disparities. Accurate forecasting for the 2028 Los Angeles Games therefore has strategic value for identifying breakthroughs and optimizing investments in disciplines and coaching.
...
\subsection{Our Work}
We organize the solution into three interconnected modules: (1) medal prediction with uncertainty estimation, (2) causal inference for coach mobility, and (3) structural insights for policy guidance. The framework aligns with the three tasks and emphasizes interpretability, robustness, and actionable recommendations.
...
Medal prediction is a continuous regression task. We evaluate linear regression, Ridge, Lasso, random forest, and gradient boosting models. Exploratory analysis reveals a right-skewed distribution of medal counts with the mean exceeding the median, indicating heavy tails and concentration among a few nations.
...
Host nations significantly outperform non-hosts. A two-sample $t$-test yields $t=15.0$ with $p<0.001$, and Cohen's $d=2.77$, indicating a large effect size. These results justify including host status as a key predictor.
...
The best-performing model (Lasso) achieves strong predictive accuracy, and Bootstrap resampling provides 95\% confidence intervals. The 2028 projection indicates a modest decline for France after losing host advantage and improvements for several rising countries.
...
We recommend priority investments for Great Britain, Brazil, and India based on performance gaps and sport importance. These recommendations align projected gains with resource efficiency.
...
\subsection{Regularization Robustness}
We evaluate sensitivity to regularization by comparing linear regression, Ridge, and Lasso models on the same standardized features. The RMSE varies narrowly from 4.58 (Linear) to 4.57 (Ridge) and 4.45 (Lasso), while $R^2$ remains stable in the range 0.945--0.948. This indicates that predictive performance is not overly sensitive to the penalty form, and that the Lasso specification provides slightly improved accuracy while preserving interpretability through feature sparsity.

\subsection{Model-Class Sensitivity}
To test robustness against model-class changes, we contrast the linear family with a nonparametric alternative. The random forest model yields $R^2=0.9297$ and RMSE $=5.20$, which is modestly worse than the linear and regularized models but still within an acceptable accuracy range. This consistency across model classes suggests that the core signal is captured by the engineered features, and the overall conclusions are stable to reasonable changes in modeling assumptions.

\subsection{Uncertainty Stability}
Bootstrap resampling is used to construct prediction intervals for medal counts, providing a direct check on forecast stability under sampling variability. The resulting intervals are sufficiently tight to preserve ranking patterns among top NOCs, supporting the robustness of the 2028 projections under data perturbations.

\section{Model Analysis}
\subsection{Strengths}
Our framework integrates prediction, uncertainty quantification, causal inference, and policy insights into a single pipeline, ensuring methodological coherence across the three tasks. By leveraging long historical records and interpretable feature engineering, the model achieves stable accuracy while remaining transparent. The outputs translate directly into actionable recommendations tailored to distinct NOC types and resource constraints.

\subsection{Weaknesses}
Coach effects are inferred indirectly due to missing personnel data, which may leave residual confounding in the estimated treatment effects. In addition, event-level heterogeneity and rule changes are only partially captured, and the prediction step assumes relatively stable geopolitical and participation conditions. These limitations indicate where future data enrichment and model extensions could improve robustness.
